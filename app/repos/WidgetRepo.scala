...
class WidgetRepoImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends WidgetRepo {
  ...
  def collection = reactiveMongoApi.db.collection[JSONCollection]("todos");
  ...
  override def save(document: BSONDocument)(implicit ec: ExecutionContext): Future[WriteResult] = {
	...
	/*upsert: Optional. If set to true, creates a new document when no document matches the query criteria. The default value is false*/
  }
